...
for(var i=0;i<movies.length;i++) {
	bigBox.innerHTML += `<div class=smallBox>
							<div id="movieImage">
							...
							<div id ="movieInfo">
								...
								<div class="likes">
									...
									<output type="number" class="output" id="${i}">0</output>
								</div>
							</div>
						</div>`;
}
...
for(var i=0;i<btn.length;i++){
  ...
  btn[i].addEventListener("click",function(){incrementLikes(this.getAttribute("id"))},false); //When passing parameter values, use an "anonymous function" that calls the specified function with the parameters: in this case the incrementLikes with the value of the button ID.
    }
...
//alternative solution for sorting items on a click

function sortItems()
{
	var switching_happened = true;
	while (switching_happened == true)
	{ //Make a loop that will continue until no switching has happened
		switching_happened = false; //start by saying: no switching has happened
		for (var i = 0; i < (smallBox.length - 1); i++)
		{
			if (Number(output[i].innerHTML) < Number(output[i + 1].innerHTML))
			{
				bigBox.insertBefore(smallBox[i + 1], smallBox[i]);
...
//TRY SORT METHOD FOR SORTING ITEMS
